...
IMG_REPO = 'https://raw.githubusercontent.com/osmanbeyoglulab/gbm_data_v2/main/correlation_comb'

st.markdown("<h2 style='text-align: center; color: black;'>L-R-Pathway-TF-Drug Correlation Heatmap for Combined Cohort</h1>", unsafe_allow_html=True)  
st.write("")

st.info("Explore spatial correlations between ligand expression, receptor expression, transcription factor activities, and pathway activities across combined glioblastoma samples. This helps reveal intercellular signaling and regulatory interactions shaping the tumor microenvironment. Interactive clustered heatmaps display Pearson correlation coefficients, allowing you to identify ligand–receptor–TF–pathway relationships that are conserved or variable across tumors. Use the search and filter options to select genes, TFs, pathways, or drugs, and explore their spatial correlations across different patients.")
...
option = "tf"
df = pd.read_csv("text_files/correlation_per_tf_names_comb.csv", header=None)
tf_list = df[0].tolist()
...
st.write("")
option = "drug"
df = pd.read_csv("text_files/correlation_per_drug_names_comb.csv", header=None)
...
st.write("")
option = "gene"
df = pd.read_csv("text_files/correlation_per_gene_names_comb.csv", header=None)
